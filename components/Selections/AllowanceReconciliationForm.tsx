...
export function AllowanceReconciliationForm({ projectId, onSuccess, onCancel }: AllowanceReconciliationFormProps) {
  const create = useCreateAllowance(projectId);
  const form = useForm<FormState>({ defaultValues: { category_name: '', allowance_budget: '', selected_cost: '' } });
  ...
  return (
    <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4">
      <div>
        ...
        <Input placeholder="e.g. Flooring" {...form.register('category_name', { required: true })} />
      </div>
      <div className="grid grid-cols-2 gap-4">
        <div>
          <label className="text-sm font-medium">Allowance Budget (CAD)</label>
          <Input type="number" step="0.01" placeholder="0.00" {...form.register('allowance_budget', { required: true })} />
        </div>
        <div>
          <label className="text-sm font-medium">Selected Cost (CAD)</label>
          <Input type="number" step="0.01" placeholder="0.00" {...form.register('selected_cost', { required: true })} />
        </div>
      </div>
