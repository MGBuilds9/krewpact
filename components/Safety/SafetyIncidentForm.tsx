...
export function SafetyIncidentForm({ onSubmit, isLoading }: SafetyIncidentFormProps) {
  const form = useForm<FormValues>({
    ...
    defaultValues: { incident_date: new Date().toISOString().split('T')[0], severity: 'low', summary: '', details: {} },
  });
      ...
      <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4">
        ...
        <FormField control={form.control} name="summary" render={({ field }) => (
          <FormItem>
            ...
            <FormControl><Input placeholder="Brief incident summary" {...field} /></FormControl>
            <FormMessage />
          </FormItem>
        )} />
        ...
        <Button type="submit" disabled={isLoading}>{isLoading ? 'Saving...' : 'Report Incident'}</Button>
      </form>
    </Form>
  );
}
