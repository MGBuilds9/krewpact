...
function formatMonthYearNullable(dateStr: string | null): string {
  if (!dateStr) return '';
...
export function ProjectHistoryCard({ project }: ProjectHistoryCardProps) {
  ...
  const locationParts = [project.project_address?.city, project.project_address?.province].filter(
    Boolean,
  );
  const location = locationParts.join(', ');

  const startFormatted = formatMonthYearNullable(project.start_date);
  const endFormatted = formatMonthYearNullable(project.end_date);
  const dateRange =
