...
function EventItem({ event }: { event: GraphEvent }): React.ReactElement {
  return (
      ...
      <div className="flex-shrink-0 text-right min-w-[4.5rem]">
        {event.isAllDay ? (
          ...
          <>
            ...
            <p className="text-xs text-muted-foreground">{formatEventTime(event.end.dateTime)}</p>
          </>
        )}
      </div>
      <div
...
export default function CalendarWidget(): React.ReactElement {
  ...
  const { data, isLoading, error } = useCalendarEvents({ startDateTime, endDateTime, top: 10 });
