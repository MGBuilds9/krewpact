...
export async function createGoodsReceipt(
  ...
  const { data: gr, error: grError } = await supabase
    ...
    .insert(grInsert)
    .select(
      'id, gr_number, po_id, division_id, location_id, received_by, created_by, received_date, notes, status, org_id, created_at, updated_at',
    )
    .single();
  ...
  const { data: lines, error: linesError } = await supabase
    ...
    .insert(lineInserts)
    .select(
      'id, gr_id, po_line_id, item_id, qty_received, unit_price, spot_id, serial_number, lot_number, condition_notes, created_at',
    );
