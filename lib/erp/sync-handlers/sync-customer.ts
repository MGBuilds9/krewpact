...
export async function syncAccount(accountId: string, _userId: string): Promise<SyncResult> {
  const supabase = await createUserClient();
  const job = await createSyncJob(supabase, 'account', accountId);
  ...
  try {
    ...
    return failJob(supabase, job.id, 'account', accountId, message);
  }
}
