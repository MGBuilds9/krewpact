...
export async function trackAIAction(entry: CostEntry): Promise<void> {
  try {
    ...
    const costCents = entry.costCents ?? estimateCostCents(entry.modelUsed, entry.inputTokens, entry.outputTokens);
    ...
    logger.warn('Failed to track AI action', { error: err instanceof Error ? err.message : String(err) });
  }
}
