...
**Production URL:** https://hub.mdmgroupinc.ca
...
| ERPNext | https://erp.mdmgroupinc.ca (via tunnel) | ERPNext admin |
...
5. Verify: `curl -s https://hub.mdmgroupinc.ca/api/health | jq .version`
...
**Diagnosis:**
```bash
curl -sv https://hub.mdmgroupinc.ca/api/health
...
**Diagnosis:**
```bash
curl -s "https://hub.mdmgroupinc.ca/api/health?deep=true" | jq .checks.supabase
```
...
**Diagnosis:**
```bash
curl -s "https://hub.mdmgroupinc.ca/api/health?deep=true" | jq '{clerk: .checks.clerk, auth_bridge: .checks.auth_bridge}'
```
...
**Diagnosis:**
```bash
curl -s "https://hub.mdmgroupinc.ca/api/health?deep=true" | jq .checks.erpnext
...
**Diagnosis:**
```bash
curl -s "https://hub.mdmgroupinc.ca/api/health?deep=true" | jq '{qstash: .checks.qstash, crons: .checks.crons, cron_last_runs: .details.cron_last_runs}'
```
...
3. Is the queue target URL correct? Should be `https://hub.mdmgroupinc.ca/api/queue/process`

**Resolution:**
1. Fix env vars if missing → redeploy
2. Re-queue stuck jobs from QStash dashboard (Dead Letter Queue → Retry)
3. For scoring/enrichment, manually trigger cron: `curl -H "Authorization: Bearer $CRON_SECRET" https://hub.mdmgroupinc.ca/api/cron/scoring`
...
**Diagnosis:**
```bash
curl -s "https://hub.mdmgroupinc.ca/api/health?deep=true" | jq .checks.sentry
```
...
# Deep health check
curl -s "https://hub.mdmgroupinc.ca/api/health?deep=true" | jq .

# Stream Vercel logs
vercel logs --follow --scope mkg-builds

# Manually trigger a cron job
curl -H "Authorization: Bearer $CRON_SECRET" https://hub.mdmgroupinc.ca/api/cron/watchdog

# Run local deep health check
npm run health:deep

# Check current deployment version
curl -s https://hub.mdmgroupinc.ca/api/health | jq .version
```
