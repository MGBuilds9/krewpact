...
- [ ] Clerk production instance configured for Supabase Third-Party Auth
- [ ] Clerk test users contain `krewpact_user_id`, `krewpact_org_id`, `division_ids`, and `role_keys` metadata
...
| 1.7 | Metadata-backed RLS | Login as `pm@krewpact-test.com`, reload dashboard                              | Data loads without RLS errors or empty-state drift | [ ]  |
...
| 5.5 | Graph mailbox send | Send an email from shared mailbox context | Email sends and CRM activity logs     | [ ]  |
