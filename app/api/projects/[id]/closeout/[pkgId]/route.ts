...
export async function GET(_req: NextRequest, { params }: { params: Promise<{ id: string; pkgId: string }> }) {
...
export async function PATCH(req: NextRequest, { params }: { params: Promise<{ id: string; pkgId: string }> }) {
  const { userId } = await auth();
  if (!userId) return NextResponse.json({ error: 'Unauthorized' }, { status: 401 });
