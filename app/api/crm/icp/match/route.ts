...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    // Fetch active leads (not deleted, not lost/won)
    ...
    // Build ICP profiles from DB rows
    const icpProfiles = icpRows.map((row) => ({
      id: row.id,
      name: row.name,
    ...
    for (const lead of leads) {
      ...
      const estimatedValue =
        typeof enrichment['estimated_value'] === 'number'
          ? enrichment['estimated_value']
          : null;
      ...
      for (const icp of icpProfiles) {
        // Reconstruct a minimal ICPProfile for scoring
        const profile: ICPProfile = {
          name: icp.name,
          description: '',
          division_id: null,
          is_auto_generated: false,
          industry_match: icp.industry_match,
          geography_match: icp.geography_match ?? { cities: [], provinces: [] },
          project_value_range: icp.project_value_range ?? { min: 0, max: 0 },
          project_types: [],
          repeat_rate_weight: icp.repeat_rate_weight,
          sample_size: 0,
          confidence_score: 0,
          avg_deal_value: 0,
          avg_project_duration_days: 0,
          top_sources: icp.top_sources,
        };
