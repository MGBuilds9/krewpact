...
import { useExpenses, useCreateExpense, useUpdateExpense, type ExpenseCreate } from '@/hooks/useExpenses';
import { useProjects } from '@/hooks/useProjects';
import { useDivision } from '@/contexts/DivisionContext';
import { toast } from 'sonner';

const CATEGORIES = ['Materials', 'Labor', 'Equipment', 'Travel', 'Meals', 'Supplies', 'Subcontractor', 'Other'];
...
export default function ExpensesPage() {
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
        ...
        <Dialog open={isOpen} onOpenChange={setIsOpen}>
          <DialogTrigger asChild>
            <Button><Plus className="h-4 w-4 mr-2" />Log Expense</Button>
          </DialogTrigger>
          <DialogContent>
            ...
            <div className="space-y-4 pt-2">
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <Label>Amount *</Label>
                  <Input type="number" step="0.01" placeholder="0.00" value={form.amount || ''} onChange={(e) => setForm({ ...form, amount: parseFloat(e.target.value) || 0 })} />
                </div>
                <div>
              ...
              <Button onClick={handleCreate} className="w-full">Create Expense</Button>
            </div>
          </DialogContent>
        </Dialog>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        <Card><CardContent className="pt-6"><div className="text-sm text-muted-foreground mb-1">Total</div><div className="text-2xl font-bold">${totalAmount.toLocaleString()}</div></CardContent></Card>
        <Card><CardContent className="pt-6"><div className="text-sm text-muted-foreground mb-1">Approved</div><div className="text-2xl font-bold text-green-600">${filteredExpenses.filter((e) => e.status === 'approved').reduce((s, e) => s + Number(e.amount), 0).toLocaleString()}</div></CardContent></Card>
        <Card><CardContent className="pt-6"><div className="text-sm text-muted-foreground mb-1">Pending</div><div className="text-2xl font-bold text-yellow-600">{filteredExpenses.filter((e) => e.status === 'submitted').length}</div></CardContent></Card>
      </div>
                ...
                <div className="flex items-center justify-between">
                  <div className="flex-1">
                    <div className="flex items-center gap-3 flex-wrap">
                      <span className="font-semibold text-lg">${Number(expense.amount).toLocaleString()}</span>
                      <Badge variant="outline">{expense.category}</Badge>
                      <Badge variant={getStatusVariant(expense.status)}>{expense.status}</Badge>
                    </div>
                    <p className="text-sm text-muted-foreground mt-1">{expense.description || 'No description'}</p>
