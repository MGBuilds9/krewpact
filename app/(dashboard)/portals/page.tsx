    ...
    <div className="space-y-6 p-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-muted-foreground">Manage external portal access for clients and trade partners.</p>
        </div>
      ...
      {isLoading ? (
        ...
        <div className="flex flex-col items-center justify-center rounded-lg border border-dashed py-16 text-center">
          ...
          <p className="mt-1 text-sm text-muted-foreground">Invite your first client or trade partner.</p>
        </div>
      ) : (
            ...
            <Card key={account.id} className="hover:shadow-md transition-shadow">
              <CardHeader className="pb-2">
                <div className="flex items-start justify-between">
                  <CardTitle className="text-base">{account.company_name ?? account.contact_name ?? account.email}</CardTitle>
                  <div className="flex items-center gap-1.5">
                    <div className={`h-2 w-2 rounded-full ${getStatusColor(account.status)}`} />
                    <span className="text-xs capitalize text-muted-foreground">{account.status}</span>
                  </div>
                </div>
              </CardHeader>
              <CardContent>
