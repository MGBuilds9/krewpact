  ...
  return (
    <div className="space-y-6 p-6">
      <div>
        ...
        <p className="text-sm text-muted-foreground">Configure permissions and send messages to portal users.</p>
      </div>
        ...
        {accounts.map((account) => (
          <Card key={account.id}>
            <CardContent className="flex items-center justify-between py-4">
              <div>
                <p className="font-medium">{account.company_name ?? account.contact_name ?? account.email}</p>
                <p className="text-sm text-muted-foreground">{account.email}</p>
              </div>
              <div className="flex gap-2">
                <Button variant="outline" size="sm" onClick={() => openDialog(account.id, 'permissions')}>
                  <Shield className="mr-2 h-4 w-4" />
                  Permissions
                </Button>
                <Button variant="outline" size="sm" onClick={() => openDialog(account.id, 'message')}>
      ...
      <Dialog open={!!mode} onOpenChange={(o) => { if (!o) { setMode(null); setSelectedId(null); } }}>
        <DialogContent>
          <DialogHeader>
            <DialogTitle>{mode === 'permissions' ? 'Edit Permissions' : 'Send Message'}</DialogTitle>
          </DialogHeader>
          {mode === 'permissions' && selectedId && (
            <PortalPermissionForm
              portalAccountId={selectedId}
              onSuccess={() => { setMode(null); setSelectedId(null); }}
              onCancel={() => { setMode(null); setSelectedId(null); }}
            />
          )}
