  ...
  return (
    <div className="space-y-4">
      <div className="flex flex-col sm:flex-row gap-3">
        ...
        <Button onClick={() => { setEditingAssembly(undefined); setDialogOpen(true); }}>
        ...
        <div className="grid gap-3">
          {assemblies.map((assembly) => (
            <Card
              ...
              onClick={() => { setEditingAssembly(assembly); setDialogOpen(true); }}
            >
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2 mb-1">
                      {assembly.assembly_code && (
                        <span className="text-xs font-mono text-muted-foreground">{assembly.assembly_code}</span>
                      )}
                      <h3 className="font-semibold truncate">{assembly.assembly_name}</h3>
                      <Badge variant="outline" className={`text-xs flex-shrink-0 border ${assembly.is_active ? 'bg-green-100 text-green-700 border-green-200' : 'bg-gray-100 text-gray-700 border-gray-200'}`}>
                    ...
                    <div className="flex flex-wrap items-center gap-x-4 gap-y-1 text-xs text-muted-foreground">
                      ...
                      {assembly.description && <span className="truncate max-w-[300px]">{assembly.description}</span>}
                    </div>
                  </div>
                </div>
              </CardContent>
