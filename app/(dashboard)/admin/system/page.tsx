...
function WebhookStatus({ status }: { status: string }) {
  const colors: Record<string, string> = { processed: 'default', failed: 'destructive', queued: 'secondary', dead_letter: 'destructive' };
  return <Badge variant={(colors[status] ?? 'outline') as 'default' | 'destructive' | 'secondary' | 'outline'} className="capitalize">{status.replace('_', ' ')}</Badge>;
}
              ...
              <CardContent className="flex items-center gap-4 py-3">
                <div className="flex-1">
                  ...
                  <p className="text-xs text-muted-foreground">
                    {wh.source} · {new Date(wh.created_at).toLocaleString('en-CA')} · {wh.attempts} attempt(s)
                  </p>
                  {wh.last_error && <p className="mt-1 text-xs text-destructive">{wh.last_error}</p>}
                </div>
                ...
                <Card key={log.id}>
                  <CardContent className="py-3">
                    <div className="flex items-center gap-3">
                      ...
                      <span className="font-mono text-sm">{log.entity_type}/{log.entity_id.slice(0, 8)}</span>
