...
export default function PrivacyPage() {
  ...
  async function advanceStatus(id: string, current: string) {
    const flow: Record<string, string> = { received: 'verified', verified: 'in_progress', in_progress: 'completed' };
    const next = flow[current];
    if (next) await update.mutateAsync({ id, status: next as 'received' | 'verified' | 'in_progress' | 'completed' | 'rejected' });
  }
    ...
    <div className="space-y-6 p-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-muted-foreground">PIPEDA compliance — access, correction, deletion, export requests.</p>
        </div>
            ...
            <Card key={req.id}>
              <CardContent className="flex items-center gap-4 py-4">
                ...
                {['received', 'verified', 'in_progress'].includes(req.status) && (
                  <Button variant="outline" size="sm" onClick={() => advanceStatus(req.id, req.status)}>
                    Advance
                  </Button>
                )}
              </CardContent>
