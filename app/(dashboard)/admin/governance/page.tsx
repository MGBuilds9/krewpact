    ...
    <div className="space-y-3">
      ...
      {isLoading ? <Skeleton className="h-24 w-full" /> : (
        <div className="space-y-1">
          {values.map((v) => (
            <div key={v.id} className="flex items-center gap-2 rounded px-2 py-1 text-sm hover:bg-muted">
      ...
      <div className="w-80 space-y-4">
        <div className="flex items-center justify-between">
          ...
          <Dialog open={open} onOpenChange={setOpen}>
            ...
            <DialogContent>
              <DialogHeader><DialogTitle>Create Reference Data Set</DialogTitle></DialogHeader>
              <ReferenceDataSetForm onSuccess={() => setOpen(false)} onCancel={() => setOpen(false)} />
            </DialogContent>
          </Dialog>
        </div>
        ...
        {isLoading ? (
          ...
          <div className="space-y-1">
            {sets.map((s) => (
              <button
                key={s.id}
                onClick={() => { setSelectedId(s.id); setSelectedName(s.set_name); }}
