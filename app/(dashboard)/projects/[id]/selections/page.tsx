    ...
    <div className="space-y-6 p-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-muted-foreground">Manage selection sheets, options, and client choices.</p>
        </div>
        <Dialog open={open} onOpenChange={setOpen}>
          ...
          <DialogContent>
            ...
            <SelectionSheetForm projectId={projectId} onSuccess={() => setOpen(false)} onCancel={() => setOpen(false)} />
          </DialogContent>
        </Dialog>
      </div>
              ...
              <CardHeader className="pb-2">
                <div className="flex items-center justify-between">
                  ...
                  <div className="flex items-center gap-1.5">
                    <div className={`h-2 w-2 rounded-full ${STATUS_COLORS[sheet.status] ?? 'bg-gray-400'}`} />
                    <span className="text-xs capitalize text-muted-foreground">{sheet.status.replace('_', ' ')}</span>
                  </div>
                </div>
              </CardHeader>
              <CardContent>
