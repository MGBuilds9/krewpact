      ...
      <Tabs defaultValue="forms">
        ...
        <TabsContent value="forms" className="space-y-4">
          <div className="flex justify-end">
            <Dialog open={openDialog === 'form'} onOpenChange={(o) => setOpenDialog(o ? 'form' : null)}>
          ...
          {loadingForms ? <LoadingSkeleton /> : (
            <div className="space-y-2">
              {forms.length === 0 && <p className="text-muted-foreground text-sm">No safety forms yet.</p>}
              {forms.map((f) => (
                <Card key={f.id}>
                  <CardContent className="flex items-center justify-between py-3 px-4">
                    ...
                    <Badge variant="outline" className="capitalize">{f.state}</Badge>
                  </CardContent>
                </Card>
              ))}
            </div>
        ...
        <TabsContent value="incidents" className="space-y-4">
          <div className="flex justify-end">
            <Dialog open={openDialog === 'incident'} onOpenChange={(o) => setOpenDialog(o ? 'incident' : null)}>
              <DialogTrigger asChild>
          ...
          {loadingIncidents ? <LoadingSkeleton /> : (
            <div className="space-y-2">
              {incidents.length === 0 && <p className="text-muted-foreground text-sm">No incidents recorded.</p>}
              {incidents.map((inc) => (
                <Card key={inc.id}>
                  <CardContent className="flex items-center justify-between py-3 px-4">
                    <div>
                    ...
                    <div className="flex gap-2">
                      <Badge variant={SEVERITY_VARIANT[inc.severity]} className="capitalize">{inc.severity}</Badge>
        ...
        <TabsContent value="talks" className="space-y-4">
          <div className="flex justify-end">
            <Dialog open={openDialog === 'talk'} onOpenChange={(o) => setOpenDialog(o ? 'talk' : null)}>
              <DialogTrigger asChild>
                <Button size="sm">
          ...
          {loadingTalks ? <LoadingSkeleton /> : (
            <div className="space-y-2">
              {talks.length === 0 && <p className="text-muted-foreground text-sm">No toolbox talks recorded.</p>}
              {talks.map((t) => (
                <Card key={t.id}>
                  <CardContent className="flex items-center justify-between py-3 px-4">
                    <div>
        ...
        <TabsContent value="inspections" className="space-y-4">
          <div className="flex justify-end">
            <Dialog open={openDialog === 'inspection'} onOpenChange={(o) => setOpenDialog(o ? 'inspection' : null)}>
              <DialogTrigger asChild>
                <Button size="sm">
          ...
          {loadingInspections ? <LoadingSkeleton /> : (
            <div className="space-y-2">
              {inspections.length === 0 && <p className="text-muted-foreground text-sm">No inspections recorded.</p>}
              {inspections.map((insp) => (
                <Card key={insp.id}>
                  <CardContent className="flex items-center justify-between py-3 px-4">
                    ...
                    <Badge variant="outline" className="capitalize">{insp.state}</Badge>
                  </CardContent>
                </Card>
              ))}
            </div>
