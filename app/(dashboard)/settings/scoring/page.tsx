...
export default function ScoringRulesPage() {
  ...
  const updateRule = useMutation({
    mutationFn: ({ id, ...data }: { id: string } & Omit<RuleFormState, 'score_impact'> & { score_impact: number }) =>
  ...
  function handleSubmitEdit() {
    if (!editingId) return;
    updateRule.mutate({ id: editingId, ...form, score_impact: parseInt(form.score_impact, 10) || 0 });
  }
    ...
    <div className="space-y-6 max-w-4xl">
      <div className="flex items-center gap-3">
        ...
        <div>
          ...
          <p className="text-muted-foreground">Configure fit, intent, and engagement scoring criteria</p>
        </div>
      </div>
                  ...
                  {editingId === rule.id ? (
                    ...
                    <div className="flex items-center justify-between py-2.5 px-3 rounded-lg border hover:bg-muted/30 transition-colors">
                      ...
                      <div className="flex items-center gap-3 flex-shrink-0 ml-4">
                        <span className={`text-sm font-bold tabular-nums ${rule.score_impact >= 0 ? 'text-green-600' : 'text-red-600'}`}>
...
function RuleForm({ form, onChange, onSubmit, onCancel, isSubmitting, submitLabel }: RuleFormProps) {
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex gap-2 justify-end">
        ...
        <Button size="sm" onClick={onSubmit} disabled={isSubmitting || !form.name || !form.field_name || !form.value}>
