    ...
    <div className="space-y-3">
      ...
      {isLoading ? (
        ...
        <div className="space-y-1">
          {permissions.map((rp) => (
            <div key={rp.id} className="flex items-center gap-2 rounded px-2 py-1 text-sm hover:bg-muted">
        ...
        {isLoading ? (
          ...
          <div className="space-y-1">
            {roles.map((role) => (
              <button
                key={role.id}
                onClick={() => { setSelectedRoleId(role.id); setSelectedRoleName(role.role_name); }}
