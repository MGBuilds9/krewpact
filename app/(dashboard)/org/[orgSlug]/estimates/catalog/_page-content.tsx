  ...
  if (isLoading)
    return (
      <div className="space-y-4">
        ...
        {['c1', 'c2', 'c3'].map((k) => (
          <Skeleton key={k} className="h-16 rounded-xl animate-pulse" />
        ))}
      </div>
    );

  const openEdit = (item?: CostCatalogItem) => {
    setEditingItem(item);
    setDialogOpen(true);
  };
    ...
    <div className="space-y-4">
      ...
      {items.length === 0 ? (
        <Card>
          <CardContent className="py-12 text-center">
            ...
            <p className="text-muted-foreground mb-4">
              Add materials, labor rates, and equipment to your cost catalog
            </p>
          </CardContent>
        </Card>
      ) : (
        <div className="grid gap-3">
          {items.map((item) => (
            <CatalogItemCard key={item.id} item={item} onClick={() => openEdit(item)} />
          ))}
        </div>
      )}
