...
export default function EstimateTemplatesPage() {
  ...
  if (isLoading) return (
    <div className="space-y-4">
      ...
      {['t1', 't2', 't3'].map((i) => <Skeleton key={i} className="h-16 rounded-xl animate-pulse" />)}
    </div>
  );

  const openEdit = (template?: EstimateTemplate) => { setEditingTemplate(template); setDialogOpen(true); };
    ...
    <div className="space-y-4">
      ...
      {filtered.length === 0 ? (
        <Card>
          <CardContent className="py-12 text-center">
            ...
            <p className="text-muted-foreground mb-4">Create estimate templates to standardize your proposals</p>
          </CardContent>
        </Card>
      ) : (
        <div className="grid gap-3">
          {filtered.map((template) => (
            <TemplateCard key={template.id} template={template} onClick={() => openEdit(template)} />
          ))}
        </div>
      )}
      <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
        <DialogContent className="max-w-2xl">
          ...
          <EstimateTemplateForm template={editingTemplate} divisionId={divId} onSuccess={() => setDialogOpen(false)} onCancel={() => setDialogOpen(false)} />
        </DialogContent>
      </Dialog>
    </div>
  );
