...
### Apr 7, 2026 (afternoon) — CRM Polish PR 2 + PR 3 + authenticated production QA pipeline via @clerk/testing

- **Changes:** Shipped PR 2 (`76c15dc4`): title metadata sweep across 35 files (6 page.tsx metadata titles stripped, 25 client-side `<title>` JSX overrides removed) + new `lib/format/currency.ts` with `formatCurrency` + `formatCurrencyCompact` (two helpers per Codex review). Shipped PR 3 (`bd0565df`): 8-item polish sweep — RBAC dedupe at the RPC source (not the hook), `lib/format/strip-tracking-chars.ts` wired into `InboxPreview`, QuickAccessToolbar context-aware split (primary action + dropdown via `primaryOn` hint), Rep Performance unassigned banner, **ISSUE-012 closed_won unification (Path A2)** via two Supabase migrations applied to prod (`20260407_001_add_closed_won_stage` + `20260407_002_migrate_contracted_won_to_closed_won` — also caught the dormant `20260317_002_opportunities_won_columns` migration that had never been applied, meaning the `/won` route was dead code in prod), inline plural fixes for DivisionComparisonCard + PipelineAgingCard. Re-ran /qa to verify and caught two PR 2 leftover misses fixed in `a5145aab`: `app/(dashboard)/org/[orgSlug]/page.tsx` (`title: 'KrewPact'` would have rendered as `KrewPact — KrewPact` via the template) and `components/CRM/PipelineValueChartInner.tsx:71` (recharts XAxis tickFormatter still using the broken `$${(v / 1000).toFixed(0)}k` pattern). **Built the new authenticated production QA pipeline (`10a0c59b`):** `npm run qa:e2e` via `playwright-qa.config.ts` + `e2e/qa/qa-auth.setup.ts` + `e2e/qa/qa-verification.spec.ts` using `@clerk/testing`. Tavily-searched for the right pattern after the existing browse + cookie-export workflow hit Clerk's WAF bot detection (visible Chromium window literally closed every time we tried to submit credentials). `clerk.signIn()` from `@clerk/testing` calls Clerk's API directly, bypasses bot detection entirely. First run: 12/12 tests passing in 17.3 seconds. Every issue from the morning baseline verified live in production with screenshot + text + console evidence. NEW findings caught: F1 — 10 inventory items render as "Unknown" name (data quality, see qa-report-krewpact-ca-2026-04-07-pr-verification.md). F2 — `/inventory` page never reaches networkidle (continuous polling). Health score 72 → 92.
- **Tests:** **5,401/5,401 vitest passing** (started day at 5,341 — added 60 new across the 3 PRs) + **12/12 authenticated E2E passing**. 0 TS errors. 26 lint warnings (all pre-existing). Build clean.
- **Next:** Verify Vercel auto-deployed `a5145aab` + `10a0c59b` (Sentry release tag in production should advance from `bd0565df` to `10a0c59b`). Address F1 (inventory Unknown items). Profile F2 (inventory slow load). Resume FeatureGate stash (restored to working tree, untouched). For any future authenticated browser QA: `npm run qa:e2e`.

### Apr 7, 2026 (morning) — CRM Polish PR 1: All Divisions sentinel + pipeline pagination fix + active projects filter

- **Changes:** /qa on production krewpact.ca found 14 confirmed issues / health score 72/100. /autoplan ran with Codex-only dual voices (subagents broken — inherited MCP context exceeds budget). Codex eng review caught **8 errors in plan v1** with file:line citations, including a NEW critical bug the QA pass missed: **ISSUE-016 — `app/api/crm/opportunities/route.ts:64` applied `range(offset, offset+limit-1)` to `view=pipeline` queries, silently capping kanban totals at default page size**. Plan rewritten to v2. **PR 1 (`b117e504`):** `contexts/DivisionContext.tsx` — `ALL_DIVISIONS_ID`, sentinel, `isAllDivisions()`, `getDivisionFilter()`, `requireConcreteDivision()` helpers + boot tolerance. `DivisionSelector` "All Divisions" dropdown. `dashboard/route.ts:23` — Active Projects filter broadened to `IN (planning, active, on_hold)`. `opportunities/route.ts:64` — pipeline view skips pagination (ISSUE-016). 22 read-filter consumers + 11 write-mutation consumers swept. 23 new tests.
- **Tests:** 5,364/5,364 passing (5,341 + 23 new).
- **Next:** PR 2 + PR 3 (completed in afternoon session).
