...
describe('mapExpenseToErp', () => {
  it('maps all fields correctly', () => {
    ...
    expect(result.total_claimed_amount).toBe(250.00);
    expect(result.total_sanctioned_amount).toBe(250.00);
  ...
  it('creates a single expense line item', () => {
    ...
    expect(expenses[0].amount).toBe(250.00);
    expect(expenses[0].tax_amount).toBe(32.50);
  });
  ...
  it('sets claimed and sanctioned amounts equal to input amount', () => {
    const result = mapExpenseToErp(makeInput({ amount: 1500.00 }));
    expect(result.total_claimed_amount).toBe(1500.00);
    expect(result.total_sanctioned_amount).toBe(1500.00);
  });
});
