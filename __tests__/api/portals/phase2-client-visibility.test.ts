...
function buildPortalMock(
  ...
  const permSet = overrides.permSet ?? {
    view_documents: true,
    approve_change_orders: true,
    view_financials: true,
  };
  const portalAccount =
    overrides.portalAccount !== undefined ? overrides.portalAccount : ACTIVE_PORTAL_ACCOUNT;
  ...
  single.mockImplementation(() => {
    ...
    if (callCount === 1)
      return Promise.resolve({
        data: portalAccount,
        error: portalAccount ? null : { message: 'Not found' },
      });
    if (callCount === 2)
      return Promise.resolve({
        data: permissions?.[0] ? { permission_set: permSet } : null,
        error: permissions?.[0] ? null : { message: 'Not found' },
      });
  ...
  in_.mockImplementation(() => ({
    ...
    then: (resolve: (v: { data: unknown; error: null }) => void) =>
      resolve({ data: changeOrders, error: null }),
  }));
  ...
  limit.mockImplementation(() => ({
    then: (resolve: (v: { data: unknown; error: null }) => void) =>
      resolve({ data: snapshots, error: null }),
  }));
