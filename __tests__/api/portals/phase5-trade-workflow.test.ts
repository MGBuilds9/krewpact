...
function makeTradeMock(
  overrides: { portalAccount?: object | null; task?: object | null; permFound?: boolean } = {},
) {
  const pa = overrides.portalAccount !== undefined ? overrides.portalAccount : ACTIVE_TRADE_PA;
  const task = overrides.task ?? {
    id: TASK_ID,
    status: 'in_progress',
    metadata: { trade_portal_id: 'pa-trade-001' },
  };
  ...
  singleMock.mockImplementation(() => {
    callCount++;
    if (callCount === 1)
      return Promise.resolve({ data: pa, error: pa ? null : { message: 'not found' } });
    if (callCount === 2)
      return Promise.resolve({ data: task, error: task ? null : { message: 'not found' } });
    return Promise.resolve({
      data: permFound ? { id: 'perm-001' } : null,
      error: permFound ? null : { message: 'not found' },
    });
  });
...
describe('PATCH /api/portal/trade/tasks/[id]/status', () => {
  ...
  it('returns 401 when unauthenticated', async () => {
    ...
    const res = await patchTaskStatus(
      makeJsonRequest(`/api/portal/trade/tasks/${TASK_ID}/status`, { status: 'done' }, 'PATCH'),
...
describe('POST /api/portal/trade/submittals', () => {
  ...
  it('returns 400 when title is empty', async () => {
    ...
    const res = await postSubmittal(
      makeJsonRequest(
        '/api/portal/trade/submittals',
        { project_id: TEST_IDS.PROJECT_ID, submittal_type: 'shop_drawing', title: '' },
        'POST',
      ),
    );
    expect(res.status).toBe(400);
  ...
  it('returns 400 for invalid submittal_type', async () => {
    ...
    const res = await postSubmittal(
      makeJsonRequest(
        '/api/portal/trade/submittals',
        { project_id: TEST_IDS.PROJECT_ID, submittal_type: 'not_a_type', title: 'Valid title' },
        'POST',
      ),
    );
    expect(res.status).toBe(400);
    ...
    const res = await postSiteLog(
      makeJsonRequest(
        '/api/portal/trade/site-logs',
        {
          project_id: TEST_IDS.PROJECT_ID,
          log_date: '2026-02-27',
          work_summary: 'Installed conduit on all circuits in panel room',
          crew_count: 4,
        },
        'POST',
      ),
  ...
  it('returns 400 for invalid log_date format', async () => {
    ...
    const res = await postSiteLog(
      makeJsonRequest(
        ...
        {
          project_id: TEST_IDS.PROJECT_ID,
          log_date: '27-02-2026', // Wrong format - should be YYYY-MM-DD
          work_summary: 'Complete enough work summary for framing crew',
          crew_count: 4,
        },
        'POST',
      ),
...
describe('Portal immutability guards', () => {
  ...
  it('in_progress task can be transitioned to done by trade partner', () => {
    ...
    const canTransition =
      task.status !== 'done' && ['todo', 'in_progress', 'blocked', 'done'].includes(newStatus);
